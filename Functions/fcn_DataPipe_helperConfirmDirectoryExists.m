function flagDirectoryExists = fcn_DataPipe_helperConfirmDirectoryExists(directoryName, varargin)
...
% INPUTS:
%
%      directoryName: a string containing the path of the temp zip
%      directory.
%
%      (OPTIONAL INPUTS)
% 
%      flagHaltIfFail: if set to true, will halt the code if directory does
%      not exist, throwing an error. Default is false, to continue even if
%      there is a failure
%
...
% OUTPUTS:
%
%      flagDirectoryExists: returns true if directory exists, false otherwise
%
% DEPENDENCIES:
%
...
% EXAMPLES:
%
%     See the script: script_test_fcn_DataPipe_helperConfirmDirectoryExists
...
if 0==flag_max_speed
    if flag_check_inputs
        ...
        % Check the directoryName to be sure it is a string
        % fcn_DebugTools_checkInputsToFunctions(directoryName, '_of_strings');
        assert(isstring(directoryName)||ischar(directoryName));
    end
end
