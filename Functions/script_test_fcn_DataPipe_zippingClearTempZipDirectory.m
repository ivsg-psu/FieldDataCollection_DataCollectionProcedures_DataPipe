% script_test_fcn_DataPipe_zippingClearTempZipDirectory.m
% tests fcn_DataPipe_zippingClearTempZipDirectory.m
...
[SUCCESS,MESSAGE,MESSAGEID] = mkdir(pwd,testDirectoryString);
if ~SUCCESS
    error('Unable to make a test directory');
end

tempZipDirectory = fullfile(pwd,testDirectoryString);

% Call the function
fcn_DataPipe_zippingClearTempZipDirectory(tempZipDirectory, (figNum));
...
% assert(istable(infoTable));
% 
% % Check variable sizes
% assert(size(infoTable,1)>=1);
% assert(size(infoTable,2)>=7);
% 
% % Check variable values
% assert(isstring(infoTable.DeviceID));
% assert(isstring(infoTable.VolumeName));
% assert(isstring(infoTable.SerialNumber));
% assert(isstring(infoTable.FileSystem));
% assert(isnumeric(infoTable.Size));
% assert(iscategorical(infoTable.SizeUnit));
% assert(isstring(infoTable.DriveType));
...
% Call the function
fcn_DataPipe_zippingClearTempZipDirectory(tempZipDirectory, ([]));
...
if 1==0
    %
        ...
        [lap_traversals, input_and_exit_traversals] = fcn_DataPipe_zippingClearTempZipDirectory(tempZipDirectory, ...
