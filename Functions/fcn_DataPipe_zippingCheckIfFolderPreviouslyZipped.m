function flags_folderWasPreviouslyZipped = fcn_DataPipe_zippingCheckIfFolderPreviouslyZipped(cellArrayOfHashFullNames, varargin)
%% fcn_DataPipe_zippingCheckIfFolderPreviouslyZipped
...
% EXAMPLES:
%
%     See the script: script_test_fcn_DataPipe_zippingCheckIfFolderPreviouslyZipped
...
flags_folderWasPreviouslyZipped = false(Nfolders,1);
for ith_folder = 1:Nfolders
    ...
    if ~isempty(hashFolderZipContents) && length(hashFolderZipContents)==(length(hashFolderAllContents)-2)
        flags_folderWasPreviouslyZipped(ith_folder,1) = true;
    end
