...
sourceRootOrSubroot = uigetdir(fullParsedBagRootDefault,'Select a folder that should be searched, including its subdirectories, for zipped hash tables.');
...
if 1==flag_processElseWhere
    destinationRootOrSubroot = uigetdir(tempZipDirectoryDefault,'Select a folder to hold temporary unzipped files during the process. These files may be up to 20GB.');
    % If the user hits cancel, it returns 0
...
directory_allVelodyneHashes = fcn_DebugTools_listDirectoryContents({sourceRootOrSubroot}, 'hashVelodyne_*', (flag_fileOrDirectory), (-1));
directory_allCamerasHashes = fcn_DebugTools_listDirectoryContents({sourceRootOrSubroot}, 'hashCameras_*', (flag_fileOrDirectory), (-1));
directory_allOusterO1Hashes = fcn_DebugTools_listDirectoryContents({sourceRootOrSubroot}, 'hashOusterO1_*', (flag_fileOrDirectory), (-1));
directory_allHashes = [directory_allVelodyneHashes; directory_allCamerasHashes; directory_allOusterO1Hashes];

fprintf(1,'\n\nFound %.0f hash folders in the parsed data folder: %s\n',length(directory_allHashes), sourceRootOrSubroot);
...
[hashFullNames, hashShortNames] = ...
    fcn_INTERNAL_extractFullAndShortNames(directory_allHashes, sourceRootOrSubroot, '');
...
fid = 1;
fcn_DebugTools_printNumeredDirectoryList(directory_allHashes, cellArrayHeaders, cellArrayValues, (sourceRootOrSubroot), (fid))
