			...
			if not os.path.exists(filename):
				...
				# Mose bag files do not contain /velodyne_points topics, keeping here just in case some old bag files need to be parsed
					...
					if flag_parse_velodyne==1:
						...
						number_scans = len(LiDAR_Bag_Time)
						batch_size = 100
						if number_scans % batch_size == 0:
							batch_number = number_scans//batch_size
						elif (number_scans % batch_size > 0) and (number_scans % batch_size < 100):
							batch_number = number_scans//batch_size + 1
						else:
							os.error("There are errors in the script")	
						print ("There are " + str(batch_number) + " of batches")		
						scan_count = 1
						batch_packet = None
						batch_count = 1
						for stamp, points, topic, scan_frame_id in vd.read_bag(bagName, config, topicName, as_pcl_structs = True):
							...
							bag_time = np.full((num_points, 1), LiDAR_Bag_Time[count_of_LiDARScan-1])
							ros_time = np.full((num_points, 1), LiDAR_Header_Time[count_of_LiDARScan-1])
							host_time = np.full((num_points, 1),int(stamp.host*10**(9)))
							device_time = np.full((num_points, 1),int(stamp.device*10**(9)))
							times_array = np.hstack((bag_time, ros_time, host_time, device_time))
							count_column = np.full((num_points,1), count_of_LiDARScan)
							points_2d = np.stack(np.array(points,dtype = object))
							
							points_withCount = np.concatenate((points_2d, count_column),axis = 1)
							num_batch_columns = np.shape(points_withCount)[1]
							if batch_packet is None:
								batch_packet = np.empty((0, num_batch_columns))

							batch_packet = np.concatenate((batch_packet, points_withCount),axis = 0)
							# Use the first scan
							if scan_count == 1:
								md5_batch = md5_scan

							if scan_count == batch_size:
								batch_hash_branch = velodyne_folder + '/' + md5_batch[0:2] + '/' + md5_batch[2:4] + '/'
								batch_ply_file =  batch_hash_branch + str(md5_batch) + '.ply'
								try:
									os.makedirs(batch_hash_branch)
								except:
									pass
								num_points_in_batch = np.shape(batch_packet)[0]
								with open(batch_ply_file, 'w') as f:
									# f.write(f"""ply
									# 	format ascii 1.0
									# 	element vertex {num_points_in_batch}
									# 	property float x
									# 	property float y
									# 	property float z
									...
									# 	property float ros_time
									# 	property float bag_time
									# 	property float host_time
									# 	property float device_time
									...
									f.write(f"""ply
										...
										end_header
										""")
									np.savetxt(f, batch_packet, fmt ="%.8f %.8f %.8f %.8f %.8f %d %d %d %d")
								batch_packet = np.empty((0, num_batch_columns))
								scan_count = 1
								batch_count += 1
							else:
						...
						if len(batch_packet)>0 and batch_packet is not None:
							batch_hash_branch = velodyne_folder + '/' + md5_batch[0:2] + '/' + md5_batch[2:4] + '/'
							batch_ply_file =  batch_hash_branch + str(md5_batch) + '.ply'
							num_points_in_batch = np.shape(batch_packet)[0]
							try:
						...
						print ("Saved " + str(batch_count) + " of  " + str(batch_number) + " PLY Files For Velodyne LiDAR")	
						File.close()
					else:
